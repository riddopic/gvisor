	...
	for _, test := range tests {
		t.Run(test.name, func(t *testing.T) {
			...
			if ep, err := addressableEndpoint.AddAndAcquirePermanentAddress(test.epAddr, stack.AddressProperties{}); err != nil {
				t.Fatalf("addressableEndpoint.AddAndAcquirePermanentAddress(%s, AddressProperties{}): %s", test.epAddr, err)
			} else {
				ep.DecRef()
			}
	...
	for _, c := range cases {
		t.Run(c.name, func(t *testing.T) {
			...
			if ep, err := addressableEndpoint.AddAndAcquirePermanentAddress(addr, stack.AddressProperties{}); err != nil {
				t.Fatalf("addressableEndpoint.AddAndAcquirePermanentAddress(%s, AddressProperties{}): %s", addr, err)
			} else {
				ep.DecRef()
			}
