...
func (e *endpoint) handleListenSegment(ctx *listenContext, s *segment) tcpip.Error {
	...
	case s.flags == header.TCPFlagSyn:
		...
		synOpts := header.TCPSynOptions{
			WS:    -1,
			TS:    opts.TS,
			TSVal: tcpTimeStamp(e.stack.Clock().NowMonotonic(), e.stack.TSOffset(s.dstAddr, s.srcAddr)),
			TSEcr: opts.TSVal,
	...
	case s.flags.Contains(header.TCPFlagAck):
		...
		// that we have used in SYNACK in order to calculate RTT.
		n.TSOffset = e.stack.TSOffset(s.dstAddr, s.srcAddr)
