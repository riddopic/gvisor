...
func (e *Endpoint) prepareWrite(preparePacket PacketFunc, opts tcpip.WriteOptions) (writeContext, tcpip.Error) {
	e.mu.RLock()
	defer e.mu.RUnlock()
	...
	pkt, err := preparePacket(
		route.NetProto(),
		route.LocalAddress(),
		route.RemoteAddress(),
		int(route.MaxHeaderLength()),
		route.RequiresTXTransportChecksum(),
	)
	if err != nil {
		route.Release()
		return writeContext{}, err
