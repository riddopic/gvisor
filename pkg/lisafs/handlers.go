...
func MountHandler(c *Connection, comm Communicator, payloadLen uint32) (uint32, error) {
	...
	if c.mounted {
		log.Warningf("connection has already been mounted at %q", c.serverImpl.Server().MountPath())
		return 0, unix.EBUSY
	}

	root, err := c.serverImpl.Mount(c.cm.getServer(mountPath))
	if err != nil {
		return 0, err
	}
...
func ChannelHandler(c *Connection, comm Communicator, payloadLen uint32) (uint32, error) {
	...
	comm.AddDonationFD(clientDataFD)
	comm.AddDonationFD(fdSock)
	resp := ChannelResp{
